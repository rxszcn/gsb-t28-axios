  ...
  function httpAdapter(config) {
    return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
      ...
      if (utils.isSpecCompliantForm(data)) {
        ...
        // Only derive the type from the body when the caller never touched
        // Content-Type; an explicit `undefined`/`false` is a deliberate removal
        // and must not be resurrected with the Blob's type.
        data.size &&
